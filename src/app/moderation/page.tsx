...
import { Calendar, Users, CheckCircle, FileQuestion, UserPlus } from "lucide-react";
...
export default async function ModerationDashboardPage() {
  ...
  const stats = [
    ...
    { label: "Users", value: users.count ?? 0, icon: Users },
    {
      ...
      value: organizerRequests.count ?? 0,
      icon: UserPlus,
    },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-3 flex-wrap">
        <Link href="/moderation/pending">
          <Button variant="outline">
            Review Pending ({pending.count ?? 0})
          </Button>
        </Link>
        <Link href="/moderation/events">
          <Button variant="outline">Manage Events</Button>
        </Link>
        <Link href="/moderation/organizer-requests">
          <Button variant="outline">
            Organizer Requests ({organizerRequests.count ?? 0})
          </Button>
        </Link>
        <Link href="/moderation/users">
          <Button variant="outline">Manage Users</Button>
        </Link>
        <Link href="/moderation/stats">
