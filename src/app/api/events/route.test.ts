...
describe("GET /api/events cursor pagination", () => {
  ...
  const buildEvent = (idx: number, startDate: string) => ({
    id: UUID[idx - 1],
  ...
  it("returns nextCursor on first page when more results exist", async () => {
    ...
    expect(cursorPayload.id).toBe(UUID[1]);
  ...
  it("returns prevCursor when requesting with cursor", async () => {
    ...
    const cursor = encodeCursor({ sortValue: "2026-02-02", id: UUID[1] });
    ...
    expect(cursorPayload.id).toBe(UUID[2]);
  ...
  it("returns prevCursor and nextCursor when requesting with before", async () => {
    ...
    const before = encodeCursor({ sortValue: "2026-02-04", id: UUID[3] });
    const request = new NextRequest(
    ...
    expect(prevPayload.id).toBe(UUID[1]);
    expect(prevPayload.sortValue).toBe("2026-02-02");
    expect(nextPayload.id).toBe(UUID[0]);
