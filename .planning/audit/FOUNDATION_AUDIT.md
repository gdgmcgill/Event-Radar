...
**Severity:** Medium · **Category:** validation · **Status:** Fixed · **Closes in phase:** 04
...
**Resolution.** **Fixed in Phase 4: plan 04-05 (`1351480`), closed out by plan 04-06.** The fallback now awaits the caller's `user_follows` read and passes `.in("user_id", ids)` an array, which is empty when the caller follows nobody, so it returns real mutual follows. The `(supabase as any)` cast is removed and `grep -rn "(supabase as any)" src/app/api/` returns 0; the one left under `src/app/` is F-072's, for Phase 5. The DEFECT suite `src/__tests__/api/events/friends-defect.test.ts` MOVED in the same commit, from `Array.isArray(passed) === false` / `{ friends: [], count: 0 }` to an array of ids and the mutual-follow result, and gained a pin for an empty follow list. Ledger rows are in `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/defect-ledger.md`. The suite was 2/5 red while unedited, 6/6 green once moved, and 3/6 red against the pre-fix route. The seam adoption before it is `fe4e9f9`. Whether the fallback should exist at all, now that `get_friends_going_to_event` is in the baseline migration, is not decided here: the fix keeps the fallback and only corrects it. Slice 1 after-evidence is in `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/slice-1-close.md`.
...
**Resolution.** **Non-visual half fixed in Phase 4; visual half DEFERRED as DI-39 (status stays Open).** Fixed by 04-10: `1150b0f` gives the six list routes one shared `EVENT_WITH_CLUB_SELECT` embed carrying the five real link columns (pin B moved), and `9083430` gives `/api/users/saved-events` the real club from the join (pin C moved); `contact_email` is deliberately not embedded (DEC-27). Not fixed: `transformEventFromDB`'s organizer fallback still fabricates a club when a row has no embed (pin A), and `/api/events/[id]` still selects `*` (pin D), so the detail page says "Hosted by" the organizer label. Shipping that half changes seeded rendering, and the 04-11 owner checkpoint was resolved by rule to `option-defer` because no owner answer was available (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/visual-fix-decision.md`). Deferred as DI-39, owner the phase owner (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/deferred-items.md`). ROADMAP criterion 2 and REFAC-10's first clause are PARTIAL on this account. See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 1 and § 2.
...
**Resolution.** **Surfacing half fixed in Phase 4; the six identity mappings DEFERRED as DI-40 (status stays Open).** Fixed by 04-07: `2db5d2a` centralized the mapping in `src/lib/eventTags.ts` with no mapped output changed, and `46731e6` added `partitionTags`, a server-side warning naming the event id and the unmapped tag, and a completeness test over `Object.values(EventTag)` that lists the six non-round-tripping members in `KNOWN_NON_ROUNDTRIP_TAGS`. ROADMAP criterion 3's tag clause is met by that alone. Not fixed: `tech`, `food`, `volunteer`, `arts`, `music` and `networking` still map to other members, so the F-081 DEFECT suite has not moved. The identity mappings change seeded badges, and the 04-11 owner checkpoint was resolved by rule to `option-defer` (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/visual-fix-decision.md`). Deferred as DI-40, owner the phase owner, with DI-37 (prototype-key lookup) and the `TAG_HIERARCHY` `competition` disagreement travelling beside it. See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 1 and § 5.
...
**Severity:** Medium · **Category:** injection · **Status:** Fixed · **Closes in phase:** 04
...
**Resolution.** **Fixed in Phase 4: plan 04-08 (`c20d59b`), shipped under DEC-32 rather than the 04-11 owner checkpoint; closed out by plan 04-11.** Search input now passes through `ilikeContainsFilter` (`src/lib/searchFilter.ts`): `%`, `_` and backslash are LIKE-escaped and the whole value is PostgREST-quoted, so a comma, parenthesis or double quote no longer splits the or() logic tree. `search-escaping-defect.test.ts` moved in the same commit; `searchFilter.test.ts` has one escape per character; the local-only probe `scripts/probes/search-escape-probe.ts` matched each special character literally (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/search-escape-probe.txt`); `GET /api/events?search=a,b` is a 200 (Playwright `FIXED F-082`). Intentional seeded delta, argued in DEC-32: a `%` or `_` search returned both approved events before and none after. Plain-text search is unchanged (events-list PRESERVE suite, unmodified). The asterisk is knowingly out of scope (PostgREST rewrites `*` to `%`; owner F-078, Phase 5), and the F-059 error echo stays open (Phase 5). See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 4.
...
**Resolution.** **Fixed in Phase 4: plan 04-09 (`ea71bb6`, DEC-25), closed out by plan 04-11.** `GET /api/events` implements the keyset cursor contract `useEvents` already spoke: order `(start_date, id)`, base64 JSON cursor decoded by `src/lib/eventCursor.ts` (400 `Invalid cursor` on anything else), `nextCursor`/`prevCursor` on every 200 body, `total` from a head count that ignores the keyset. `src/app/api/events/route.test.ts` runs un-skipped (PRESERVE); `pagination-contract-defect.test.ts` moved; Playwright tests traverse both seeded approved events by cursor (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/playwright.pagination.txt`). Load More now renders in production when a filtered result exceeds 30 (not reachable on the two-event seed). **Follow-up, owner Phase 8:** keyset efficiency at scale wants a partial composite index `on public.events (start_date, id) where status = 'approved' and deleted_at is null`; today only single-column `idx_events_start_date` exists, so a tie on `start_date` falls back to filtering. That is a migration, which orchestrator decision 2 keeps out of Phase 4 (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/pagination-contract.txt` § 4; T-04-09-02 accepted). See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 4.
...
**Severity:** Low · **Category:** dead-code · **Status:** Fixed · **Closes in phase:** 04
...
**Resolution.** **Fixed in Phase 4: plans 04-06 (`a037d95`), 04-07 (`2db5d2a`) and 04-10 (`9083430`), closed out by plan 04-11.** 04-06 moved the six analytics fixture rows from `event_date` to `start_date` and corrected the events-webhook docblock (comment only). 04-07 reworded the last stale reference, the `transformEventFromDB` docblock in `src/lib/tagMapping.ts`, when the tag mapping moved to `src/lib/eventTags.ts`. 04-10 replaced the false "Clubs table does not exist" comment in `src/app/api/events/[id]/route.ts` GET. Validation criterion met: the stale event-date census (`grep -rn "event_date\|event_time" src/ supabase/` outside `_archive_pre_baseline`) prints 0 lines, and the three analytics suites pass (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/floor.phase-after.txt` blocks 1 and 14). The authoritative columns are `start_date`/`end_date` (AUDIT-19). See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 1, criterion 2.
...
**Severity:** Low · **Category:** config · **Status:** Fixed · **Closes in phase:** 04
...
**Resolution.** **PARTIALLY CLOSED in Phase 2; `closes_in_phase` moved 02 -> 03 for the remainder.** Closed: the four suites blocked by an uninstalled `@testing-library/react` now execute. `jest-environment-jsdom` and the three testing-library packages were installed and Jest split into two projects with an explicit routing rule — `src/hooks/useEvents.test.ts` is a `.ts` file that needs a DOM, so the rule is not 'by file extension' (`0556444`, plan 02-08). The suite count went 16 of 21 executing to 22 of 23, and tests went 220 passed / 36 skipped to **278 passed / 5 skipped**. NOT closed, and both halves were correctly predicted by this finding: (1) `src/app/api/events/route.test.ts` remains skipped — no install revives it, because it asserts a cursor-pagination contract the handler has no cursor concept for; reviving it against current behaviour would freeze a possible defect as the specification, so the contract question is handed to REFAC-10 in Phase 4 (`evidence/skipped-suite-disposition.md`). (2) `tsconfig.json` still excludes every test file, so the whole-program type-check hole that made F-050 invisible is still open; that belongs with the generated types and the type-drift check in Phase 3. **Reassigned `03` -> `04` by plan 03-08. STILL OPEN on both clauses, and Phase 3 closed neither.** Phase 2 named Phase 3 as the natural home; Phase 3 declined it deliberately and says so rather than letting the assignment lapse. **(1) Zero skipped suites.** One suite remains skipped — `src/app/api/events/route.test.ts`, the contract-drift suite — and no install revives it: it asserts a cursor-pagination contract the handler has no cursor concept for, so reviving it against current behaviour would freeze a possible defect as the specification (`.planning/phases/02-dependency-and-runtime-stabilization/evidence/skipped-suite-disposition.md`). That contract question is `REFAC-10`, Phase 4. **(2) Test files type-checked.** `tsconfig.json` still excludes `**/*.test.ts`, measured by plan 03-06 at roughly **86 errors across 10 files**, almost all mechanical, and deliberately kept out of the type-regeneration commits — 86 mechanical edits in the same diff a reviewer is asked to certify line by line would have made that diff unreviewable (`.planning/phases/03-refactor-foundations-schema-truth-and-the-seam-kit/evidence/type-fixes-note.md` section 3.2). Both clauses land in Phase 4. Registered as `DI-24` in `.planning/phases/03-refactor-foundations-schema-truth-and-the-seam-kit/evidence/deferred-items.md`. **Phase 4, plan 04-06 (`9530d35`): the type-check clause is MET.** `tsconfig.json` no longer excludes `**/*.test.ts` or `**/*.test.tsx`. With them included, `npx tsc --noEmit` reported 71 errors in 7 test files. All of them were fixed inside the tests, and no route signature changed (`git diff -- src/app src/lib src/server` is empty). It now exits 0, and so does the type-check inside `next build` (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/tsc-tests-included.txt`, `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/slice-1-close.md`). The skipped-suite clause is still NOT met: `src/app/api/events/route.test.ts` remains skipped (4 pending), and it belongs to REFAC-10, plan 04-09. Status stays Open. (Plan 04-06 also merged this record's two `resolution` keys into this one. The Phase 3 paragraph above had been written as a second, duplicate key, so JSON parsers kept only the Phase 2 paragraph and the Phase 3 note never reached FOUNDATION_AUDIT.md.) **Phase 4, plan 04-09 (`ea71bb6`): the skipped-suite clause is MET.** `src/app/api/events/route.test.ts` was rewritten against DEC-25, the cursor contract `useEvents` already spoke (F-083), and was not revived against the old route (T-01-11-04). It runs as a PRESERVE suite of 19 tests. The one remaining skipped test, in `src/__tests__/api/events/get-events.test.ts`, was revived because its skip reason was false: the route has always called `.eq("status", "approved")`. `npx jest --ci` reports 50 of 50 suites and 717 passed, with no skipped suite and no skipped test (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/pagination-contract.txt`, `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/defect-ledger.md`). Both clauses are now met. The status flip is 04-11's. **Fixed in Phase 4, both clauses (closed out by plan 04-11).** Type-check clause: `9530d35` (04-06) removed the test-file exclusion from `tsconfig.json`; `npx tsc --noEmit` type-checks every test file with zero diagnostics. Skipped-suite clause: `ea71bb6` (04-09) rewrote `src/app/api/events/route.test.ts` against the DEC-25 contract (running, PRESERVE) and revived the last skipped test in `get-events.test.ts`. At the phase after-floor `npx jest --ci` reports 50 of 50 suites and 0 skipped tests (`.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/floor.phase-after.txt` block 1). See `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/PHASE-4-COMPLETION.md` § 5.
...
**Severity:** Low · **Category:** performance · **Status:** Fixed · **Closes in phase:** 04
...
**Resolution.** **Fixed in Phase 4: plan 04-05 (`d40dee4`), closed out by plan 04-06.** RSVP GET now counts with two parallel `select("id", { count: "exact", head: true })` reads, one for `status = going` and one for `status = interested`, under `Promise.all`. An error from either read logs once under the existing "Error fetching RSVPs:" context and returns the unchanged `serverError("fetch RSVPs")` body. There is no schema change and no DB function (DEC-23), and `total === going + interested` is kept. The DEFECT suite `src/__tests__/api/events/rsvp-count-defect.test.ts` MOVED in the same commit: 4 assertions, with rows in `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/defect-ledger.md`. It was red against the fix while unedited, green once moved, and red again against the pre-fix route as a control. `rsvp-characterization.test.ts` passed unmodified before and after the fix commit, which is the validation criterion. Slice 1 after-evidence is in `.planning/phases/04-slices-1-2-saved-events-rsvp-and-the-event-read-path/evidence/slice-1-close.md`: the floor is at or above the before-floor, and Playwright is 37/37 from a clean reset, including both RSVP count tests.
