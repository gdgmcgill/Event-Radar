...
- **Phase 2 complete (2026-09-15):** Node 24 pinned (`engines`, `.nvmrc`, CI), Jest 30 the single runner (Vitest removed), 11 dead packages removed, `next` 16.3.5 closes 7 advisories, `middleware.ts` → `proxy.ts` rename characterized before and after, CI runs tests plus an unsuppressed `npm audit --audit-level=high --omit=dev` gate (0 critical / 0 high in production deps), Renovate configured (bot not yet installed), CycloneDX SBOM committed, clean-room `npm ci` proven. Security gate closed at `threats_open: 0` (`02-SECURITY.md`); CI observed green and the proxy rename smoke-tested on a Vercel preview. Deploy blocker found and fixed: `public/brand` was a symlink no Vercel build could copy since May 2026.
- **Phase 3 complete (2026-09-16):** 44 legacy migrations archived by pure `git mv`; one baseline dumped from production over a read-only transport (AR-12 discharged per read); `supabase db reset --local` replays and `db diff --linked` was empty at baseline; three post-baseline migrations (trigram indexes, invitee/membership RLS, `compute_user_scores` cron) exist locally only — production repair deferred to Phase 8 (`repair-preflight.md` holds the exact command), so **`supabase db push` must not run before then**. Types regenerated from `--local` with a red-then-green CI drift gate; 45 of 47 `(supabase as any)` casts removed (DEC-22: the last two hide F-071 and F-072/F-073 — every admin audit write has been silently rejected in production because `admin_audit_log.admin_email` never reached it). `src/server/` seam kit with an ESLint import boundary and a shrink-only ratchet, applied to zero routes. Auth callback characterized (8 tests, 9 mutation cycles). Deterministic seed with a hard-refusing loader; Playwright persona harness (27 tests) green locally and on CI. Code review actioned: invitation policies fixed forward, harness hardened, CI least-privilege; F-074..F-078 registered (anon-executable `SECURITY DEFINER` functions; fuzzy search has never worked — every search falls back to `ILIKE`). Floor: Jest 358/5, pgTAP 86, lint 0 errors, baseline tool 22/0. REFAC-04 and REFAC-07 recorded partial.
- **Correction (Phase 3):** no staging Supabase project exists under this account (research and 03-07 both measured it), contradicting the earlier "a staging Supabase project already exists" note below; REFAC-07's staging clause is therefore partial and Stage 4 must provision or designate one before certification datasets can load anywhere but local.
...
- **Environments:** local Supabase configured (`supabase/config.toml`); ~~a staging Supabase project already exists~~ — **not found in Phase 3** (see Correction above); production on Supabase + Vercel. The Vercel project is not git-linked; deploys are explicit `vercel deploy`, previews sit behind Deployment Protection.
...
| Stage 1 audit chooses the single test runner | Neither Jest nor Vitest has been inventoried for which tests actually run; decide on evidence | ✓ Jest 30 (Phase 2 removed Vitest; two Jest projects, node + jsdom) |
| Use the existing staging Supabase project for certification datasets | Already provisioned; avoids standing up a new environment | ✗ Invalidated — no staging project exists (Phase 3); Stage 4 must provision one |
| Stage 1 is strictly read-only | Findings must be captured with evidence before anything changes, so fixes can be validated against a baseline | ✓ Held (Phase 1); Phases 2–3 also left production unwritten |
| Migration history reconciled by archive + production baseline, never by renaming | REFAC-01 forbids renames; 18 production versions had no local file at all | ✓ Phase 3 (D-01); production `migration repair` deferred to Phase 8 (D-02, `defer-to-phase-8`) |
| Keep the last two Supabase casts rather than change behaviour in a typing plan | Both hide registered defects (F-071, F-072/F-073) whose fixes are behaviour changes | ✓ Phase 3 (DEC-22); REFAC-04 recorded partial, owners Phase 4/5 |
| Club owners write their own club through the elevated door, behind the owner gate, rather than through new owner UPDATE policies (F-087) | An owner policy is a migration, and migrations are local-only until the Phase 8 repair; the door works in production on deploy and keeps `status` unwritable through a column whitelist | ✓ Phase 5 (DEC-41, 05-10 `4531ee2`); rule-resolved, no owner present |
| Split F-028: four personalized routes answer anonymous callers 401 in Phase 5, the other three move with the cache work | The four had user-gated clients; the rsvp GET, club events and notification count are tied to REFAC-19's cache inversion | ✓ Phase 5 (DEC-39, 05-06 `7ff08c1`); F-028 Open at Phase 6 |
| Phase 5 migrations stay local-only until the Phase 8 migration-history repair; migration-dependent findings stay Open | Marking F-006/F-007/F-008/F-016 Fixed on local proof would claim a production state never measured | ⚠ Phase 5 (DEC-57): two Criticals open in production until DI-23 or an owner-authorized early apply |
| The Upstash package checkpoint was resolved by rule ("use 2.0.8"), and production without a store degrades loudly instead of refusing to start | No owner answered the blocking checkpoint; a storeless boot failure would take production down for a rate limiter, which is not an authorization control; `RATE_LIMIT_REQUIRE_DISTRIBUTED=true` restores the fail-closed boot | ⚠ Phase 5 (DEC-59, 05-18); owner countersignature and Upstash provisioning pending before any push to `main` |
| The supabase-js 2.116.0 minor waits for typed admin payloads rather than being forced | The worktree proof found four admin writes that fail `tsc` on 2.116.0; retyping them is a behaviour decision on admin write paths | ⚠ Phase 5 (DEC-56, 05-19): DI-25 partial, carried as DI-53 to Phase 6 |
...
---
*Last updated: 2026-09-16 after Phase 3 (Refactor Foundations — Schema Truth and the Seam Kit) completion*
