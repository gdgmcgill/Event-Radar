...
- [x] **REFAC-11**: Slice 3 (auth/session/ban/onboarding): `getUser()` at every authorization decision, middleware is advisory-only and the ban check fails closed, the onboarding guard cannot be bypassed by direct API calls, env-var non-null assertions are replaced with validated config
- [x] **REFAC-12**: Slice 4 (club authorization/membership): the 19 hand-rolled club-membership checks collapse into `requireClubRole`, cross-club access attempts return 403 at the authz ring and are denied at the RLS ring
- [x] **REFAC-13**: Slice 5 (admin/moderation/service-role containment): every fail-open endpoint fails closed, `verifyAdmin()` guards every admin route, every remaining service-role use goes through `src/server/db/elevated/` with a registered justification, `/api/admin/*` is included in rate limiting
...
| REFAC-11 | Phase 5 | Complete (05-19), measured clause by clause in .planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/PHASE-5-COMPLETION.md § 3. It was PARTIAL at the slice-3 close (05-08) on one clause, "middleware is advisory-only", because a banned user holding `admin` was refused on /api/admin/* only by the proxy (DI-48). DI-48 closed at 35 of 35 admin arms: every arm runs requireActiveUser(ctx) before requireRole(ctx, "admin") (DEC-58; 05-13 90819ee, 9f0e5b5; 05-14 4cf4928), pinned by BAN_GUARDED_ARMS in src/__tests__/api/admin/admin-guard-defect.test.ts. The other four clauses stay met on the phase after-floor (getSession( only in the non-gating health route; ban and profile fail closed; onboarding from the database with requireOnboarded on every write arm; env non-null assertions 0; floor.phase-after.txt blocks 14-17). Read-side residual, stated: the two admin page layouts and the moderation reviews GET admin path decide by role only, so the proxy still refuses a banned admin there (DI-55, Phase 6), consistent with DEC-34 (GET arms carry no ban guard) |
| REFAC-12 | Phase 5 | Complete (05-11), measured clause by clause at the slice-4 close (.planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/slice-4-close.md § 7). (1) The hand-rolled checks collapse into `requireClubRole`: research § C counts the "19" as 17 gate and flag sites; 18 `requireClubRole(` call sites under src/app/api (#15's composite has two arms), and 0 gate-site `club_members` reads left in the 13 gate files (floor.slice-4-after.txt censuses 20 and 21; 05-09 a566410, 05-10 a6fd637 and d8cf84e). (2) Cross-club attempts get 403 at the authz ring: 12 cross_club_attacker 403s byte-equal to § C in e2e/specs/club-authorization.spec.ts, plus the 107-row PRESERVE table's non-member rows, green in the slice-4 full run (76/0). (3) Denied at the RLS ring: supabase/tests/database/060-club-tenant-isolation.test.sql, 30 assertions in both directions across events, clubs, club_members and club_invitations, green unseeded and seeded, and proven to bite by the mutation check and the WITH CHECK (true) regression (5d9c22e; schema-push-slice-4.txt). The events INSERT half (F-008) is proven on the local stack and reaches production with the DI-23 migration repair (Phase 8). The club-table attacker denials it pins (clubs UPDATE and INSERT, club_members UPDATE, DELETE and INSERT, club_invitations INSERT) rest on baseline policies production already has, and none of them depends on the local-only migrations |
| REFAC-13 | Phase 5 | PARTIAL (05-19), measured clause by clause in .planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/PHASE-5-COMPLETION.md § 3. Met: `verifyAdmin()` guards every admin route, through its replacement requireRole(ctx, "admin") after requireActiveUser(ctx) at all 35 admin arms (DEC-44, DEC-58; the helper file deleted by b8e172e; floor.phase-after.txt blocks 16 and 24); `/api/admin/*` is included in rate limiting (600 GET / 120 mutation per IP, path and minute; 408064d; policy.test.ts). Unmet clauses: "every fail-open endpoint fails closed" (FO-01 calculate-popularity closed by 4cf4928, FO-03 and FO-05 closed in slice 3, but FO-02, the reminder cron comparing against `Bearer undefined`, is still open: F-002, Phase 6) and "every remaining service-role use goes through src/server/db/elevated/ with a registered justification" (every non-cron use does, allow-list 25 → 2, but the two cron routes still import the service module directly; floor.phase-after.txt block 23). Both unmet halves are the cron routes REFAC-14 (Phase 6) owns |
| REFAC-14 | Phase 6 | Pending |
| REFAC-15 | Phase 6 | Pending |
| REFAC-16 | Phase 6 | Pending |
| REFAC-17 | Phase 5 | Complete (05-19; first recorded by 05-17), measured clause by clause in .planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/PHASE-5-COMPLETION.md § 3. Exposure assessed against the installed @supabase/ssr 0.7.0 cookie defaults (evidence/csrf-assessment.md); protection added: the proxy refuses cross-site /api/* mutations with 403 before session work (f245d58, src/server/csrf.ts, 41 unit cases, e2e/specs/csrf-origin.spec.ts). Stated residual: /invites/[token] accepts an invitation on GET, outside /api/ (Low; the attacker must hold the token; DI-41, owner decision). If the owner reads that route as exposure remaining, this requirement is PARTIAL for exactly it |
| REFAC-18 | Phase 5 | PARTIAL (05-19; 05-18 had recorded Complete), measured clause by clause in .planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/PHASE-5-COMPLETION.md § 3. Met in the code: rate limiting runs behind RateLimitStore (408064d) and counts in Upstash when UPSTASH_REDIS_REST_* or KV_REST_API_* is configured (941bee7, DEC-59), unit-tested with a mocked client. Unmet clause: "so it works across serverless instances". No store is provisioned (DI-42), production counts per instance from the memory store with a loud boot error, and src/server/ratelimit/upstash.contract.test.ts is skipped. It closes when the owner provisions Upstash and the contract test runs green against it |
