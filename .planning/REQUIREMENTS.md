...
| REFAC-11 | Phase 5 | PARTIAL (05-08, measured clause by clause at the slice-3 close; see .planning/phases/05-slices-3-5-auth-club-authorization-admin-containment/evidence/slice-3-close.md § 7). Met: `getUser()` at every authorization decision (the only production `getSession(` is the non-gating api/health, pinned by getsession-gate.test.ts); the ban check fails closed (proxy e2d6d3a; requireActiveUser on all 39 non-admin write arms, 05-06 aa50ff6, 05-07 aa2191e and c620d16; legacy helper deleted); the onboarding guard cannot be bypassed by direct API calls (requireOnboarded plus DEC-34's two wizard exemptions; e2e 403 in ban-and-onboarding-ring.spec.ts); env non-null assertions replaced with validated config (census 0, 05-04, 05-05). Unmet clause: "middleware is advisory-only" holds for every non-admin arm but not for the admin arms' ban decision, where a banned user holding `admin` is refused only by the proxy (DI-48, owner slice 5, 05-12/05-13) |
